...
const createClient = async (req, res) => {
  try {
    ...
    if (!selectMembership)
      return res
        .status(400)
        .send({ status: false, message: "selectMembership is required" });

    selectMembership = clientsAllData.selectMembership =
      selectMembership.trim();

    if (selectMembership == "")
      return res.status(400).send({
        status: false,
        message: "Please Enter selectMembership value",
      });

    if (typeof selectMembership != "string")
      return res.status(400).send({
        status: false,
        message: "selectMembership should be in String",
      });
    ...
    if (!companyName)
      return res
        .status(400)
        .send({ status: false, message: "companyName is required" });

    if (typeof companyName != "string")
      return res
        .status(400)
        .send({ status: false, message: "companyName should be in String" });
    ...
    if (companyName == "")
      return res
        .status(400)
        .send({ status: false, message: "Please Enter companyName value" });
    ...
    if (!inputNumber)
      return res
        .status(400)
        .send({ status: false, message: "inputNumber is mandatory" });

    if (typeof inputNumber != "string") {
      return res
        .status(400)
        .send({ status: false, message: " please send proper inputNumber" });
    }
    ...
    if (inputNumber.length !== 10 && inputNumber.length !== 15)
      return res.status(400).send({
        status: false,
        message: " please send proper GSTNo or IEC Code",
      });

    if (inputNumber.length == 15) {
      if (!gstRegex.test(inputNumber))
        return res
          .status(400)
          .json({ valid: false, message: "Invalid GST number." });
    }

    if (inputNumber.length === 10) {
      if (!iecRegex.test(inputNumber))
        return res
          .status(400)
          .json({ valid: false, message: "Invalid IEC number." });
    }

    if (!numberOfEmployees)
      return res
        .status(400)
        .send({ status: false, message: "numberOfEmployees is required" });

    if (typeof numberOfEmployees != "number")
      return res.status(400).send({
        status: false,
        message: "numberOfEmployees should be in number",
      });
    ...
    if (numberOfEmployees == "")
      return res.status(400).send({
        status: false,
        message: "Please Enter numberOfEmployees value",
      });
    ...
    if (!websiteAdd)
      return res
        .status(400)
        .send({ status: false, message: "websiteAdd is required" });

    if (typeof websiteAdd != "string")
      return res
        .status(400)
        .send({ status: false, message: "websiteAdd should be in String" });
    ...
    if (websiteAdd == "")
      return res
        .status(400)
        .send({ status: false, message: "Please Enter websiteAdd value" });

    if (!validation.validateWebsite(websiteAdd))
      return res
        .status(400)
        .send({ status: false, message: "please provide valid website" });
    ...
    if (!address1)
      return res
        .status(400)
        .send({ status: false, message: "address1 is required" });

    if (typeof address1 != "string")
      return res
        .status(400)
        .send({ status: false, message: "address1 should be in String" });
    ...
    if (address1 == "")
      return res
        .status(400)
        .send({ status: false, message: "Please Enter address1 value" });

    if (!address2)
      return res
        .status(400)
        .send({ status: false, message: "address2 is required" });

    if (typeof address2 != "string")
      return res
        .status(400)
        .send({ status: false, message: "address2 should be in String" });
    address2 = clientsAllData.address2 = address2.trim();

    if (address2 == "")
      return res
        .status(400)
        .send({ status: false, message: "Please Enter address2 value" });
    //_________________________________________________________________________
    if (address3) {
      if (typeof address3 != "string")
        return res
          .status(400)
          .send({ status: false, message: "address3 should be in String" });
      ...
      if (address3 == "")
        return res
          .status(400)
          .send({ status: false, message: "Please Enter address3 value" });
    }
    ...
    if (address4) {
      if (typeof address4 != "string")
        return res
          .status(400)
          .send({ status: false, message: "address4 should be in String" });
      address4 = clientsAllData.address4 = address4.trim();

      if (address4 == "")
        return res
          .status(400)
          .send({ status: false, message: "Please Enter address4 value" });
    }
    ...
    if (typeof country != "string")
      return res
        .status(400)
        .send({ status: false, message: "country should be in String" });
    country = clientsAllData.country = country.trim();

    if (country == "")
      return res
        .status(400)
        .send({ status: false, message: "Please Enter country value" });
    ...
    if (typeof state != "string")
      return res
        .status(400)
        .send({ status: false, message: "state should be in String" });
    state = clientsAllData.state = state.trim();

    if (state == "")
      return res
        .status(400)
        .send({ status: false, message: "Please Enter state value" });
    ...
    if (typeof pinCode != "number")
      return res
        .status(400)
        .send({ status: false, message: "pinCode should be in number" });

    if (pinCode == "")
      return res
        .status(400)
        .send({ status: false, message: "Please Enter pinCode value" });

    if (!validation.validatePincode(pinCode))
      return res
        .status(400)
    ...
    if (!businessCategory)
      return res
        .status(400)
        .send({ status: false, message: "businessCategory is required" });

    if (typeof businessCategory != "string")
      return res.status(400).send({
        status: false,
        message: "businessCategory should be in String",
      });

    businessCategory = clientsAllData.businessCategory =
      businessCategory.trim();

    if (businessCategory == "")
      return res.status(400).send({
        status: false,
        message: "Please Enter businessCategory value",
      });
    ...
    if (!howDidYouKnowAboutUs)
      return res
        .status(400)
        .send({ status: false, message: "howDidYouKnowAboutUs is required" });

    if (typeof howDidYouKnowAboutUs != "string")
      return res.status(400).send({
        status: false,
        message: "howDidYouKnowAboutUs should be in String",
      });

    howDidYouKnowAboutUs = clientsAllData.howDidYouKnowAboutUs =
      howDidYouKnowAboutUs.trim();

    if (howDidYouKnowAboutUs == "")
      return res.status(400).send({
        status: false,
        message: "Please Enter howDidYouKnowAboutUs value",
      });
    ...
    if (!surName)
      return res
        .status(400)
        .send({ status: false, message: "surName is required" });

    if (typeof surName != "string")
      return res
        .status(400)
        .send({ status: false, message: "surName should be in String" });
    ...
    if (surName == "")
      return res
        .status(400)
        .send({ status: false, message: "Please Enter surName value" });
    ...
    if (typeof role != "string")
      return res
        .status(400)
        .send({ status: false, message: "role should be in String" });
    ...
    if (role == "")
      return res
        .status(400)
        .send({ status: false, message: "Please Enter role value" });
    ...
    if (!telephoneNo)
      return res
        .status(400)
        .send({ status: false, message: "telephoneNo is required" });

    if (typeof telephoneNo != "string")
      return res
        .status(400)
        .send({ status: false, message: "telephoneNo should be in string" });

    if (telephoneNo == "")
      return res
        .status(400)
        .send({ status: false, message: "Please Enter telephoneNo value" });

    if (!validation.validateTelephoneNo(telephoneNo))
      return res
        .status(400)
        .send({ status: false, message: "please provide valid telephone No" });
    //___________________________________PhoneNo______________________________________
    if (!phoneNo)
      return res
        .status(400)
        .send({ status: false, message: "phoneNo is required" });

    if (typeof phoneNo != "number")
      return res
        .status(400)
        .send({ status: false, message: "phoneNo should be in number" });

    if (phoneNo == "")
      return res
        .status(400)
        .send({ status: false, message: "Please Enter phoneNo value" });
    ...
    if (!registeredBank)
      return res
        .status(400)
        .send({ status: false, message: "registeredBank is required" });

    if (typeof registeredBank != "string")
      return res
        .status(400)
        .send({ status: false, message: "registeredBank should be in String" });
    ...
    if (registeredBank == "")
      return res
        .status(400)
        .send({ status: false, message: "Please Enter registeredBank value" });
    ...
    if (!accNo)
      return res
        .status(400)
        .send({ status: false, message: "accNo is required" });

    if (typeof accNo != "string")
      return res
        .status(400)
        .send({ status: false, message: "accNo should be in String" });
    ...
    if (accNo == "")
      return res
        .status(400)
        .send({ status: false, message: "Please Enter accNo value" });

    let ifsc = IFSCCode;
    if (!ifsc)
      return res
        .status(400)
        .send({ status: false, message: "ifsc is required" });

    if (typeof ifsc != "string")
      return res
        .status(400)
        .send({ status: false, message: "ifsc should be in String" });
    ...
    if (ifsc == "")
      return res
        .status(400)
        .send({ status: false, message: "Please Enter ifsc value" });

    if (typeof throughEmail != "boolean")
      return res
        .status(400)
        .send({ status: false, message: "throughEmail should be in boolean" });

    if (typeof throughCalls != "boolean")
      return res
        .status(400)
        .send({ status: false, message: "throughCalls should be in boolean" });

    let clientCreated = await clientModel.create(clientsAllData);
    return res.status(201).send({
      status: true,
      message: "client got created ",
      data: clientCreated,
    });
  } catch (error) {
...
const resetPasswordClient = async (req, res) => {
  ...
  let some = await clientModel.findOneAndUpdate(
    { email: user.email },
...
const apporveMail = async (req, res) => {
  try {
    ...
    if (!foundData)
      return res
        .status(400)
        .send({ status: false, message: "Not Authorized to proceed !!!" });
...
const getCompanyDetails = async (req, res) => {
  try {
    ...
    if (!clientId)
      return res
        .status(400)
        .send({ status: false, message: "Please Enter clientId value" });
    ...
    return res.status(200).send({
      status: true,
      message: "here's company Details",
      data: clientCompanyDetails,
    });
  } catch (error) {
    return res.status(500).send({ status: false, message: error.message });
  }
...
const getClientPersonalInfo = async (req, res) => {
  try {
    ...
    return res.status(200).send({
      status: true,
      message: "here's personal Details",
...
const changePassword = async (req, res) => {
  try {
    ...
    let { email, currentPassword, newPassword, confirmPassword } =
      changePasswordInfo;
    if (!clientId)
      return res
        .status(400)
    ...
    if (!currentPassword)
      return res
        .status(400)
        .send({ status: false, message: "currentPassword is required" });

    if (typeof currentPassword != "string")
      return res.status(400).send({
        status: false,
        message: "currentPassword should be in String",
      });

    if (currentPassword == "")
      return res
        .status(400)
        .send({ status: false, message: "Please Enter currentPassword value" });
    ...
    if (!isClientExists)
      return res
        .status(404)
        .send({ status: false, message: "client doesn't exists " });
    ...
    if (!newPassword)
      return res
        .status(400)
        .send({ status: false, message: "newPassword is required" });

    if (typeof newPassword != "string")
      return res
        .status(400)
        .send({ status: false, message: "newPassword should be in String" });
    ...
    if (newPassword == "")
      return res
        .status(400)
        .send({ status: false, message: "Please Enter newPassword value" });
    if (!validation.validatePassword(newPassword))
      return res.status(400).send({
        status: false,
        message:
    ...
    if (typeof confirmPassword != "string")
      return res.status(400).send({
        status: false,
        message: "confirmPassword should be in String",
      });
    ...
    if (confirmPassword == "")
      return res
        .status(400)
        .send({ status: false, message: "Please Enter confirmPassword value" });
    if (!validation.validatePassword(confirmPassword))
      return res.status(400).send({
        status: false,
        message:
    ...
    let changePasswordData = await clientPasswordChangeModel.create(
      changePasswordInfo
    );
    ...
    return res.status(200).send({
      status: true,
      message:
        "Our Team Will contact you and withing 24 hours you will be notified !!!",
      messageForfronted: `${isClientExists.companyName} has  send request for change password his email ${isClientExists.email}!!! `,
...
const commercialDir = async (req, res) => {
  try {
    ...
    if (!isAvailable)
      return res.status(404).send({ status: false, message: "no data found " });
    if (isAvailable.selectMembership === "Digital User")
      return res
        .status(400)
        .send({ status: false, message: "Members Only!!!" });
    ...
    if (!ownersName)
      return res
        .status(400)
        .send({ status: false, message: "ownersName is required" });

    if (typeof ownersName != "string")
      return res
        .status(400)
        .send({ status: false, message: "ownersName should be in String" });
    ...
    if (ownersName == "")
      return res
        .status(400)
        .send({ status: false, message: "Please Enter ownersName value" });
    ...
    if (!establishmentYear)
      return res
        .status(400)
        .send({ status: false, message: "establishmentYear is required" });

    if (typeof establishmentYear != "string")
      return res.status(400).send({
        status: false,
        message: "establishmentYear should be in string",
      });
    ...
    if (establishmentYear == "")
      return res.status(400).send({
        status: false,
        message: "Please Enter establishmentYear value",
      });
    //__________________________Mobile No_______________________
    if (!mobileNo)
      return res
        .status(400)
        .send({ status: false, message: "mobileNo is required" });

    if (typeof mobileNo != "string")
      return res
        .status(400)
        .send({ status: false, message: "mobileNo should be in string" });
    ...
    if (mobileNo == "")
      return res
        .status(400)
        .send({ status: false, message: "Please Enter mobileNo value" });
    ...
    if (!companyAdd)
      return res
        .status(400)
        .send({ status: false, message: "companyAdd is required" });

    if (typeof companyAdd != "string")
      return res
        .status(400)
        .send({ status: false, message: "companyAdd should be in String" });
    ...
    if (companyAdd == "")
      return res
        .status(400)
        .send({ status: false, message: "Please Enter companyAdd value" });

    //__________________________product_______________________
    if (!companyProduct)
      return res
        .status(400)
        .send({ status: false, message: "companyProduct is required" });

    if (typeof companyProduct != "string")
      return res
        .status(400)
        .send({ status: false, message: "companyProduct should be in String" });
    ...
    if (companyProduct == "")
      return res
        .status(400)
        .send({ status: false, message: "Please Enter companyProduct value" });
    //__________________________activity_______________________
    if (!companyActivity)
      return res
        .status(400)
        .send({ status: false, message: "companyActivity is required" });

    if (typeof companyActivity != "string")
      return res.status(400).send({
        status: false,
        message: "companyActivity should be in String",
      });
    ...
    if (companyActivity == "")
      return res
        .status(400)
        .send({ status: false, message: "Please Enter companyActivity value" });

    let dirCreated = await comercialDirectory.create(dirInfo);
    return res.status(201).send({
      status: true,
      message: "directory got created ",
      data: dirCreated,
    });
  } catch (error) {
    return res.status(500).send({ status: false, message: error.message });
  ...
  try {
    ...
    if (subCatagory) {
      if (typeof subCatagory != "string")
        return res
          .status(400)
          .send({ status: false, message: "subCatagory should be in String" });

      if (subCatagory == "")
        return res
          .status(400)
          .send({ status: false, message: "Please Enter subCatagory value" });
    }
    ...
    if (howDidYouKnowAboutUs) {
      ...
      howDidYouKnowAboutUs = updateData.howDidYouKnowAboutUs =
        howDidYouKnowAboutUs.trim();
    ...
    if (facebook) {
      if (typeof facebook != "string")
        return res
          .status(400)
          .send({ status: false, message: "facebook should be in String" });
      facebook = updateData.facebook = facebook.trim();

      if (facebook == "")
        return res
          .status(400)
          .send({ status: false, message: "Please Enter facebook value" });
    ...
    if (linkedIn) {
      if (typeof linkedIn != "string")
        return res
          .status(400)
          .send({ status: false, message: "linkedIn should be in String" });
      linkedIn = updateData.linkedIn = linkedIn.trim();

      if (linkedIn == "")
        return res
          .status(400)
          .send({ status: false, message: "Please Enter linkedIn value" });

      if (!validation.validateLinkedIn(linkedIn))
        return res.status(400).send({
          status: false,
          message: "please provide valid  linkeIn link",
        });
    }
    ...
    if (twitter) {
      if (typeof twitter != "string")
        return res
          .status(400)
          .send({ status: false, message: "twitter should be in String" });
      twitter = updateData.twitter = twitter.trim();

      if (twitter == "")
        return res
          .status(400)
          .send({ status: false, message: "Please Enter twitter value" });

      if (!validation.validateTwitter(twitter))
        return res.status(400).send({
          status: false,
          message: "please provide valid  twitter link",
        });
    }
    ...
    return res
      .status(200)
      .send({ status: true, message: "data will update within 24 hours !!!" });
  } catch (error) {
    return res.status(500).send({ status: false, message: error.message });
  }
};
...
const updatePersonalDetails = async (req, res) => {
  try {
    ...
    if (phoneNo) {
      ...
      //     return res
      //       .status(400)
      //       .send({ status: false, message: "phone number should be in number" });

      if (phoneNo == "")
        return res
          .status(400)
          .send({ status: false, message: "Please Enter phone number value" });
  ...
  try {
    ...
    if (emails) {
      if (typeof emails != "boolean")
        return res
          .status(400)
          .send({ status: false, message: "emails should be in boolean" });
    }
    if (posts) {
      if (typeof posts != "boolean")
        return res
          .status(400)
          .send({ status: false, message: "posts should be in boolean" });
    }
    if (calls) {
      if (typeof calls != "boolean")
        return res
          .status(400)
          .send({ status: false, message: "calls should be in boolean" });
    }
    if (campaigns) {
      if (typeof campaigns != "boolean")
        return res
          .status(400)
          .send({ status: false, message: "campaigns should be in boolean" });
    }
    if (awards) {
      if (typeof awards != "boolean")
        return res
          .status(400)
          .send({ status: false, message: "awards should be in boolean" });
    }
    if (weeklyBullets) {
      if (typeof weeklyBullets != "boolean")
        return res.status(400).send({
          status: false,
          message: "weeklyBullets should be in boolean",
        });
    }
    if (eventUpdates) {
      if (typeof eventUpdates != "boolean")
        return res.status(400).send({
          status: false,
          message: "eventUpdates should be in boolean",
        });
    }
    if (sponsorship) {
      if (typeof sponsorship != "boolean")
        return res
          .status(400)
          .send({ status: false, message: "sponsorship should be in boolean" });
    }
    if (magzine) {
      if (typeof magzine != "boolean")
        return res
          .status(400)
          .send({ status: false, message: "magzine should be in boolean" });
    }
    if (newsLetters) {
      if (typeof newsLetters != "boolean")
        return res
          .status(400)
          .send({ status: false, message: "newsLetters should be in boolean" });
    }

    let dataCreated = await clientMarketingModel.create(marketingInfo);
    res
      .status(200)
      .send({ status: true, message: "marketing Data", data: dataCreated });
  } catch (error) {
    return res.status(500).send({ status: false, message: error.message });
  }
};
